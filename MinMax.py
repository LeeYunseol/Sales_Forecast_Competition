...
data_df_scaled = pd.DataFrame(data=data_scaled, columns=['Store', 'year','Thanksgiving', 'WeekOfYear','Weekly_Sales'])
...
test = test[['Store', 'year','Thanksgiving', 'WeekOfYear']]

test_scaled = scaler_for_test.transform(test)
data_df_scaled2 = pd.DataFrame(data=test_scaled, columns=['Store', 'year','Thanksgiving', 'WeekOfYear'])
